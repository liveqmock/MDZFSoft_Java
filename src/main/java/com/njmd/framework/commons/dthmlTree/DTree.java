package com.njmd.framework.commons.dthmlTree;
...
public class DTree
{
	 ...
	 * 保存该树的所有节点
	 */
	private Map<String, DTreeNode> nodeMap = new HashMap<String, DTreeNode>();;
	 ...
	 */
	private Map<String, ArrayList<String>> parentNodeMap = Collections.synchronizedMap(new LinkedHashMap<String, ArrayList<String>>());// new
																																		// HashMap<String,
																																		// ArrayList<String>>();
	...
	public DTree(List<DTreeNode> nodeList)
	{

		addNodes(nodeList);
	}

	public DTree(List<DTreeNode> nodeList, String showRootId)
	{
	 ...
	 * 
	 */
	public void addNode(DTreeNode treeNode)
	{
	 ...
	 * 
	 */
	public void addNodes(List<DTreeNode> treeNodeList)
	{
		if (treeNodeList != null)
		{
			for (DTreeNode treeNode : treeNodeList)
			{
				addNode(treeNode);
			}
		}
	...
	public void removeNode(String nodeId)
	{
		...
		{
			DTreeNode treeNode = nodeMap.get(nodeId);
	...
	public void setNodeChecked(String nodeId)
	{
		if (nodeMap.containsKey(nodeId))
		{
			DTreeNode node = nodeMap.get(nodeId);
		...
		if (nodeMap.containsKey(nodeId))
		{
			...
			if (parentNodeMap.containsKey(pid))
			{
				...
				for (String cid : hmChildren)
				{
					...
					{
						DTreeNode child = nodeMap.get(cid);
						if (!child.isHidden())
			...
			if (isChildrenHidden)
			{
				...
				{
					DTreeNode parent = nodeMap.get(pid);
	...
	public void setExpectNodeHidden(List<String> nodeIds)
	{
		if (nodeIds != null)
		{
			for (DTreeNode node : nodeMap.values())
			{
