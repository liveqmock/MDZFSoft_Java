...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0">
		<tr>
			<td class="page-title"><img src="${ctx}/admin/images/page_title.gif" alt="title"> &nbsp;&nbsp;
			组织管理&gt;&gt; 菜单管理</td>
		</tr>
	</table>
	<br>
	<table width="100%" border="0" align="center" cellpadding="3" cellspacing="0">
		<tr>
			<td ><a href="javascript:window.location.reload();">刷 新</a></td>
			<td align="left">
					<input type="button" class="short-button" value="删除所选" onclick="doRestBatchDelete('${ctx}/restful/sysmenu','ids',this)" />
					<input type="button" class="short-button" value="添加菜单" onclick="add('${resultObject.permissionId}')">
			</td>
		</tr>
		<tr>
			...
			<td valign="top">
					<fieldset style="width: 95%; padding: 10px"><legend> 菜单列表 </legend>
					<table id="tableList" width="100%" cellspacing="1" align="center" class="tablesorter">
						<thead>
							<tr align="center">
								<th class="{sorter: false}" width="4%"><input class="checkbox" type="checkbox" name="operid"
									onclick="javascript:selectAll('id',this.checked)"></th>
								<th width="40%">菜单名</th>
								<th width="10%">状态</th>
								<th width="50%">操作</th>
							</tr>
							...
							<c:if test="${not empty resultList}">
								<c:forEach var="menu" items="${resultList}" varStatus="status">
									<tr align="center" style="border: 1px;">
										<td><input class="checkbox" type="checkbox" name="id" value="${menu.permissionId}" /></td>
										<td>
										...
										<td><c:if test="${menu.status=='0'}">
											<font color="red">无效</font>
										</c:if> <c:if test="${menu.status=='1'}">
							有效
						</c:if></td>
										<td><a href="${ctx}/menu/view?menuId=${menu.permissionId}" class="view">查看</a> 
											<a href="${ctx}/menu/edit?menuId=${menu.permissionId}" class="edit">编辑</a> 
											<a href="#" class="delete" onclick="del('${menu.permissionId}')">删除</a> 
											<a href="${ctx}/menu/up?menuId=${menu.permissionId}" class="delete" >上移</a>
											<a href="${ctx}/menu/down?menuId=${menu.permissionId}" class="delete" >下移</a>
									</tr>
								</c:forEach>
							</c:if>
